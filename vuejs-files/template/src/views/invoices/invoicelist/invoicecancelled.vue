							...
							<div class="card card-table"> 
								<div class="card-body">
									<div class="table-responsive">
										<table class="table table-stripped table-hover datatable" id="invoicecancelled">
											...
											<tbody>
												<tr v-for="item in invoicecancelled" :key="item.id">
													<td>
...
<script>
	import invoicecancelled from '../../../assets/json/invoicecancelled.json';
...
// Select 2
	if ($('.select').length > 0) {
		$('.select').select2({
			...
			util.datatable('#invoicecancelled')
		},
		methods: {
			loadImg(imgPath) {
